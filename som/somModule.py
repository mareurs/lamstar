...
#            norm = 0
    ...
    def findSmallBMU(self,data):
        ...
        bestIndex = 0
        for i in range(len(self.nodes)):
            norm = self.calculateDistance(self.nodes[i].getWeights()[5:-5,5:-5],data)
#            if self.debug > 2:
#                print('norm = ',norm)
            if norm < best:
                best = norm
    ...
    def calculateDistance(self, data1, data2):
        ...
        return np.sqrt(distance)/15#/(data1.shape[0]*data1.shape[1])
...
#                raw_input("Enter")
        ...
        if(bestDistance > 0.3):                           #If the distance is bigger than 20% 
#            self.addDefaultNode(example)
            
            #Randomize where to put the new node so they don't all go on the same side
...
#        learningRate = 1 * np.exp(-float(self.noOfTrains)/1000)
        ...
        for i in range(self.radius):
            ...
            self.nodes[tmpidx].setWeights(self.nodes[tmpidx].getWeights()+proximity*learningRate*(example-self.nodes[tmpidx].getWeights()))
#        where = 'query'
